...
class Trading:
    ...
    def apply_strategy(self, pairs, strategy='fixed_beta', entry_mult=1, exit_mult=0, 
                      test_mode=False, split_date='2010-01-01'):
        """Applies trading strategy to identified pairs."""
        sharpe_results = []
        cum_returns = []
        ...
        for i, pair in enumerate(pairs):
            ...
            y = pair_info['Y_test'] if test_mode else pair_info['Y_train'][split_date:]
            x = pair_info['X_test'] if test_mode else pair_info['X_train'][split_date:]

            if strategy == 'fixed_beta':
                summary, sharpe, balance_summary = self.threshold_strategy(
                    y=y, x=x, beta=pair_info['coint_coef'],
                    entry_level=entry_mult,
                    exit_level=exit_mult
                )
                ...
                print('Only fixed_beta strategy currently available')
                exit()
    ...
    def _calculate_max_drawdown(self, balance):
        ...
        dd_period = round((peak_idx - trough_idx) / 78)  # Approximation
